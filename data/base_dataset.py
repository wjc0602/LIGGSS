...
def get_transform(opt, params, method=Image.BICUBIC, normalize=True, toTensor=True):
    ...
    if 'resize' in opt.preprocess_mode:
        ...
        transform_list.append(transform.Resize(osize, mode=method))
    elif 'scale_width' in opt.preprocess_mode:
        transform_list.append(transform.Lambda(
    ...
    if normalize:
        transform_list += [transform.ImageNormalize(mean=[0.5], std=[0.5])]
    return transform.Compose(transform_list)


def normalize():
    return transform.ImageNormalize(mean=[0.5], std=[0.5])
