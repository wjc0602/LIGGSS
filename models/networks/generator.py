...
class SPADEGenerator(BaseNetwork):
    ...
    def execute(self, input_semantics, real_image, ref_semantics, ref_image, z=None):
        seg = input_semantics

        if self.opt.use_vae:
            # we sample z from unit normal and reshape the tensor
            if z is None:
                z = jt.randn(input_semantics.size(0), self.opt.z_dim)
            x = self.fc(z)
...
class Pix2PixHDGenerator(BaseNetwork):
    ...
    def execute(self, input_semantics, z=None):
        return self.model(input_semantics)